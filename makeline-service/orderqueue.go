...
func getOrdersFromQueue() ([]Order, error) {
	...
	if orderQueueHostName != "" && useWorkloadIdentityAuth == "true" {
		...
		orderQueuePassword := os.Getenv("ORDER_QUEUE_PASSWORD_MAKELINE")
		if orderQueuePassword == "" {
			log.Printf("ORDER_QUEUE_PASSWORD_MAKELINE is not set")
			return nil, errors.New("ORDER_QUEUE_PASSWORD_MAKELINE is not set")
		}
